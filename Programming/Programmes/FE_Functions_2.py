...
def SteerCamObstacle (Sign, Plus):
    ...
    if Cam_val[0] == 0:
        ...
        target = target if (target < 110 and target >-110) else 110 * target/abs(target)
...
def FEDrive(sec):
    global LineCount, ColorCondition
    ColorCondition = 1
    Timer_line.reset()
    LineCount = 0
